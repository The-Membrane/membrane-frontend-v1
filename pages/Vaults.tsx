    ...
    function createDepositElements(forVaults: boolean) {
        ...
        return(<>
           ...
           <div className="value value-menu-dropdown" onClick={handleOpen}>
            ...
            {menuLabel === "Value" ? 
                <>
                {currentfunctionLabel === "deposit" && walletQTYs.osmo > 0 || (contractQTYs.osmo??0) > 0 ? <div className={positionQTYs.osmo > 0 ?  "" : "low-opacity"}>${ (positionQTYs.osmo * +prices.osmo) > 1000 ? (((positionQTYs.osmo * +prices.osmo)/1000) ?? 0).toFixed(2)+"k" : ((positionQTYs.osmo * +prices.osmo) ?? 0).toFixed(2)}</div> : null}
            ...
            : menuLabel === "Util" ? 
                <>
                {currentfunctionLabel === "deposit" && walletQTYs.osmo > 0 || (contractQTYs.osmo??0) > 0 ? <div className={positionQTYs.osmo > 0 ?  "" : "low-opacity"}>{(debtCaps.osmo * 100).toFixed(2)}%</div> : null}
